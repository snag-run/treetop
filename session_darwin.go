...
// then reads each one's working directory and open regular files with `lsof`.
// If `ps` can't run at all, detection reports supported == false; with `ps`
// available but no sessions, it reports supported with an empty scan.
func scanSessions() sessionScan {
	...
	// whatever we got regardless of the exit status.
...
// and the regular files they hold open. The -F format prints one field per
// line, tagged by a leading letter: f<fd>, t<type>, n<name>, repeating per
// descriptor. A descriptor is a cwd when its fd is "cwd"; it's an open file when
// its fd is a number and its type is REG — mirroring what /proc/<pid>/fd lists
// on Linux (numbered descriptors only, not mmap'd text or libraries).
